 ...
 */
									...
									$throw_exceptions = AsException::THROW_ALL )
	{
		...
		elseif( isset( $email ) )
		{
			...
			if( isset( $new_password ) && !isset( $confirmed_password ) && !isset( $lastname ) && !isset( $firstname ) 
			&& !isset( $address ) && !isset( $postal_code ) && !isset( $city ) && !isset( $phone ) )
			{
 ...
 */
	public function change_password( $old_password, $new_password, $confirmed_password, $throw_exceptions = AsException::THROW_ALL )
	{
		if( isset( $old_password ) 
			&& isset( $new_password ) && isset( $confirmed_password ) 
			&& ( $this->validate_password( $old_password, $new_password, $confirmed_password ) === true ) )
		{
 ...
 */
	protected function validate_email( $email, $throw_exceptions = AsException::THROW_ALL )
	{
		...
		{
			$exceptions[ 'email' ] = 'Ugyldig e-postadresse!';
		}
