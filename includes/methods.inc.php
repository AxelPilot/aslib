...
function shutdown() {
    ...
    if ($error['type'] === E_ERROR) {
        ...
        redirect('index.php?error=' . "We apologize, but a system error has occured. Please try again later.");
    }
}
...
function is_loggedIn($ajax = false) {
    ...
    if (!isset($_SESSION['user_ID']) || !isset($_SESSION['firstname']) || !isset($_SESSION['lastname']) || !isset($_SESSION['admin'])) {
        if (!$ajax) {
            new cms\Login(get_filename_with_url_params());
        }
        return false;
    } else {
        return true;
